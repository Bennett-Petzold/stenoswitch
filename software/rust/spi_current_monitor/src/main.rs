...
pub const CC1: mcp_defs::ChannelId = mcp_defs::CH2;
pub const CC2: mcp_defs::ChannelId = mcp_defs::CH3;
pub const CC_CHANNELS: [mcp_defs::ChannelId; 2] = [CC1, CC2];

pub const CHRG_DIVH: mcp_defs::ChannelId = mcp_defs::CH1;
pub const CHRG_DIVL: mcp_defs::ChannelId = mcp_defs::CH0;

pub const fn adc_to_voltage(adc_val: mcp_defs::AdcVal) -> Voltage {
...
fn read_channel(spi: &mut Spidev, sel: &LineHandle, id: mcp_defs::ChannelId) -> mcp_defs::AdcVal {
    sel.set_value(0).unwrap(); // Enable the MCP3204 SPI.
    ...
    spi.transfer(&mut SpidevTransfer::read_write(
        &mcp_defs::request(id),
        &mut read_buf,
    ))
    .unwrap();

    sel.set_value(1).unwrap(); // Disable the MCP3204 SPI.
    let _ = spi.write_all(&[0]); // Guarantee spacing before next request.
    ...
    println!("Read {id:?} as RAW {read_buf:?}, U16 {adc_val}, MEASURE {voltage}V");
...
fn read_cc(spi: &mut Spidev, sel: &LineHandle) -> AmpLimit {
    let adc_val = CC_CHANNELS
        .iter()
        .map(|channel| read_channel(spi, sel, *channel))
        .max()
...
fn read_current_limit(spi: &mut Spidev, sel: &LineHandle) -> f32 {
    const UPPER_DIV_OHMS: f32 = 10_000.0;

    let divh = read_channel(spi, sel, CHRG_DIVH);
    let divl = read_channel(spi, sel, CHRG_DIVL);
...
fn main() {
    ...
    // TODO: Use a "CURRENT_SQLITE" var to connect to a sqlite database
    // Check bool CC_LINES in table USB_POLL and set update callback
    // While that's enabled, continually poll for allowed voltage -> DB
    // Check bool CUR_LIMIT in table USB_POLL and set update callback
    // While that's enabled, continually poll for the set current limit -> DB
    // Shared lib for setting up the /tmp/ database.
    // Some kind of condvar notify setup for callback changes
    //
    // This monitoring will only be requested while USB is connected

    loop {
        println!("CC Lines: {:?}", read_cc(&mut spi, &sel));
        println!(
            "Current limit = {} amps",
            read_current_limit(&mut spi, &sel)
        );
